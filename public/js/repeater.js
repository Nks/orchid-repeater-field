...
/* harmony import */ var _orchid_js_controllers_application_controller__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ~orchid/js/controllers/application_controller */ "./vendor/orchid/platform/resources/js/controllers/application_controller.js");
...
var _default = /*#__PURE__*/function (_ApplicationControlle) {
  ...
  function _default() {
    ...
    _defineProperty(_assertThisInitialized(_this), "sortableInstance", null);
  ...
  _createClass(_default, [{
    ...
    value: function prepareTemplate() {
      ...
      if (templateElement) {
        var config = squirrelly__WEBPACK_IMPORTED_MODULE_3__.defaultConfig;
        config.autoEscape = false;
        this.template = squirrelly__WEBPACK_IMPORTED_MODULE_3__.compile(templateElement.innerHTML, config);
      }
    ...
    value: function fetchFields() {
      ...
      var fieldName = this.repeaterFieldTarget.name;
      var repeaterData = this.getRepeaterData();
      var values = JSON.parse(this.data.get('value'));
      this.contentTarget.classList.add('loading');
      axios__WEBPACK_IMPORTED_MODULE_1___default().post(this.data.get('url'), {
        values: values,
        repeater_name: fieldName,
        layout: this.data.get('layout'),
        repeater_data: repeaterData
      }).then(function (response) {
        if (!_this2.template && response.data.template) {
          var element = document.createElement('template');
          element.innerHTML = response.data.template.trim();
          var template = element.content.firstChild;
          var config = squirrelly__WEBPACK_IMPORTED_MODULE_3__.defaultConfig;
          config.autoEscape = false;
          _this2.template = squirrelly__WEBPACK_IMPORTED_MODULE_3__.compile(template.innerHTML, config);
        }

        if (!_this2.template) {
          _this2.alert('Unexpected error', "Error fetching repeater field template for ".concat(_this2.options.title, " (").concat(_this2.options.name, ")."), 'danger');

          return;
        }

        if (response.data.fields) {
          response.data.fields.forEach(function (content, index) {
            if (_this2.options.max === null || index < _this2.options.max) {
              _this2.blocksTarget.insertAdjacentHTML('beforeend', _this2.template({
                name: _this2.blocksTarget.dataset.containerKey,
                content: content,
                block_key: index,
                block_count: "".concat(_this2.options.title, " ").concat(index + 1)
              }));
            }
          });
        }
    ...
    value: function initDragDrop() {
      ...
      this.sortableInstance = sortablejs__WEBPACK_IMPORTED_MODULE_0__["default"].create(this.blocksTarget, {
        handle: '.card-handle',
        animation: 150,
    ...
    value: function collapse(event) {
      var currentBlock = event.currentTarget.closest('.collapse-switch');
    ...
    value: function addBlock(currentBlock) {
      ...
      var blocksCount = this.blocksTarget.querySelectorAll(':scope > .repeater-item').length;
      var num = event.detail.blocksNum || 1;
      var repeaterData = this.getRepeaterData();

      if (this.options.max && blocksCount >= this.options.max) {
        this.alert(this.data.get('error-title'), this.data.get('max-error-message'));
        return;
      }
      ...
      axios__WEBPACK_IMPORTED_MODULE_1___default().post(this.data.get('url'), {
        ...
        if (r.data.fields) {
          r.data.fields.forEach(function (content, index) {
            var compiledTemplate = _this4.template({
              name: _this4.blocksTarget.dataset.containerKey,
              content: content,
              block_key: index,
              block_count: "".concat(_this4.options.title, " ").concat(index + 1)
            });
    ...
    value: function deleteBlock(event) {
      ...
      if (this.options.min && blocksCount <= this.options.min) {
        this.alert(this.data.get('error-title'), this.data.get('min-error-message'));
        return;
      }
    ...
    value: function sort() {
      var self = this;
      var blocks = this.blocksTarget.querySelectorAll(':scope > .repeater-item');
      blocks.forEach(function (block, currentKey) {
        ...
        fields.forEach(function (field) {
          ...
          var originalName = "[".concat(repeaterNameKey.replace('.', ''), "]");
          ...
          var inputs = field.querySelectorAll('input[type="hidden"]');
          ...
          if (inputs.length) {
            inputs.forEach(function (input) {
              ...
              var resultInputName = "".concat(input.closest('.repeaters_container').dataset.containerKey, "[").concat(input.closest('.repeater-item').dataset.sort, "]").concat(originalName);
              input.setAttribute('name', resultInputName);
            });
          }

          var resultName = "".concat(field.closest('.repeaters_container').dataset.containerKey, "[").concat(field.closest('.repeater-item').dataset.sort, "]").concat(originalName);
      ...
      if (this.hasRepeaterBlockCountTarget) {
        this.repeaterBlockCountTargets.forEach(function (content, index) {
          content.innerHTML = "".concat(self.options.title, " ").concat(index + 1);
        });
      }
    ...
    key: "getRepeaterData",
    value: function getRepeaterData() {
      return this.data.get('ajax-data') ? JSON.parse(this.data.get('ajax-data')) : null;
    }
  }]);

  return _default;
}(_orchid_js_controllers_application_controller__WEBPACK_IMPORTED_MODULE_2__["default"]);
...
/***/ "./vendor/orchid/platform/resources/js/controllers/application_controller.js":
/*!***********************************************************************************!*\
  !*** ./vendor/orchid/platform/resources/js/controllers/application_controller.js ***!
  \***********************************************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
/******/ 	__webpack_require__.O(undefined, ["css/repeater"], () => (__webpack_require__("./resources/js/app.js")))
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, ["css/repeater"], () => (__webpack_require__("./resources/sass/app.scss")))
